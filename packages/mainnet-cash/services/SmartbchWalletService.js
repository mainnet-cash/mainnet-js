...
const smartbchBalance = ({ balanceRequest }) => new Promise(
  async (resolve, reject) => {
    try {
      const wallet = await _smartbch.walletFromId(balanceRequest.walletId);
      if (!wallet) {
...
const smartbchCreateWallet = ({ walletRequest }) => new Promise(
  async (resolve, reject) => {
    try {
      const resp = await _smartbch.createWalletResponse({ ...walletRequest });
      resolve(Service.successResponse({ ...resp }));
    } catch (e) {
      reject(
        Service.rejectResponse(e, e.status || 500)
...
const smartbchDepositAddress = ({ serializedWallet }) =>
  new Promise(async (resolve, reject) => {
    try {
      const wallet = await _smartbch.walletFromId(serializedWallet.walletId);
...
const smartbchMaxAmountToSend = ({ smartBchMaxAmountToSendRequest }) =>
  new Promise(async (resolve, reject) => {
    try {
      const wallet = await _smartbch.walletFromId(smartBchMaxAmountToSendRequest.walletId);
...
const smartbchSend = ({ smartBchSendRequest }) => new Promise(
  async (resolve, reject) => {
    try {
      const wallet = await _smartbch.walletFromId(smartBchSendRequest.walletId);
      if (!wallet) {
        throw Error("Could not derive wallet");
      }
...
const smartbchSendMax = ({ smartBchSendMaxRequest }) =>
  new Promise(async (resolve, reject) => {
    try {
      const wallet = await _smartbch.walletFromId(smartBchSendMaxRequest.walletId);
      if (!wallet) {
        throw Error("Could not derive wallet");
      }
...
const smartbchSignedMessageSign = ({ createSignedMessageRequest }) => 
  new Promise(async (resolve, reject) => {
    try {
      let wallet = await _smartbch.walletFromId(createSignedMessageRequest.walletId);
      if (!wallet) {
        throw Error("Could not derive wallet");
      }
...
const smartbchSignedMessageVerify = ({ verifySignedMessageRequest }) => 
  new Promise(async (resolve, reject) => {
    try {
      ...
      let resp, wallet
      wallet = await _smartbch.walletFromId(args.walletId);
      if (!wallet) {
        throw Error("Could not derive wallet");
      }
