...
import { FaucetContract } from "./FaucetContract";
  ...
  test.skip("Test deploying faucet contract with tokens", async () => {
    ...
    await wallet.send([{ address: contract.getDepositAddress(), value: 1, unit: "bch" }], {}, { gasPrice: ethers.BigNumber.from(10 ** 10), gasLimit: 1e7});
    expect((await contract.balance()).eq(contractTargetValue.add(10n ** 18n)));

    const bobWallet = await RegTestSmartBchWallet.newRandom();
    const sendResponse = await (await contract.send([Utils.zeroAddress()], [bobWallet.getDepositAddress()], [ethers.BigNumber.from(2000)], { gasPrice: ethers.BigNumber.from(10 ** 10), gasLimit: 1e7})).wait();

    const newBalance: ethers.BigNumber = await contract.balance();
    expect(newBalance.eq(contractTargetValue.add(10n ** 18n).sub(2000))).toBe(true);
    expect(await (await bobWallet.provider!.getBalance(bobWallet.address!)).toBigInt()).toBe(2000n);

    // should throw if not enough funds to send
    await expect((await contract.send([Utils.zeroAddress()], [bobWallet.getDepositAddress()], [ethers.BigNumber.from(10n ** 20n)], { gasPrice: ethers.BigNumber.from(10 ** 10), gasLimit: 1e7})).wait()).rejects.toThrow();
    ...
    const tokenSendResult = await wallet.sep20.send([{ address: contract.getDepositAddress(), tokenId: result.tokenId, value: 10 }], { gasPrice: 10 ** 10 });

    const contractWallet = await RegTestSmartBchWallet.watchOnly(contract.getDepositAddress());
    ...
    expect((await contractWallet.sep20.getBalance(result.tokenId)).value.toNumber()).toBe(10);

    // should throw if not enough tokens to send
    await expect((await contract.send([result.tokenId], [bobWallet.getDepositAddress()], [ethers.BigNumber.from(10n ** 20n)], { gasPrice: ethers.BigNumber.from(10 ** 10), gasLimit: 1e7})).wait()).rejects.toThrow();


    // invoke contract send to transfer 1 token to bob
    const response = await (await contract.send([result.tokenId], [bobWallet.getDepositAddress()], [ethers.BigNumber.from(10n ** 2n)], { gasPrice: ethers.BigNumber.from(10 ** 10), gasLimit: 1e7})).wait();

    // contract shall now have 9 tokens, bob 1 token
    expect((await contractWallet.sep20.getBalance(result.tokenId)).value.toNumber()).toBe(9);
    expect((await bobWallet.sep20.getBalance(result.tokenId)).value.toNumber()).toBe(1);
    ...
    const destroyResponse = await (await contract.destroy({ gasPrice: ethers.BigNumber.from(10 ** 10), gasLimit: 1e7})).wait();
