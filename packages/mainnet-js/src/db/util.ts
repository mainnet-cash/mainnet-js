...
export function getSslConfig() : sslConfigI {
  ...
  const ca = process.env.DATABASE_SSL_CA ? Buffer.from(process.env.DATABASE_SSL_CA, "base64").toString("ascii") : undefined
  const key = process.env.DATABASE_SSL_KEY ? Buffer.from(process.env.DATABASE_SSL_KEY, "base64").toString("ascii") : undefined
  const cert = process.env.DATABASE_SSL_CERT ? Buffer.from(process.env.DATABASE_SSL_CERT, "base64").toString("ascii") : undefined
  let ssl: sslConfigI = {
