...
test("Should get electrum settings from defaults", async () => {
  expect(config.getDefaultServers(Network.MAINNET)).toBe(primary.mainnetServers);
  expect(config.getDefaultServers(Network.TESTNET)).toBe(primary.testnetServers);
  expect(config.getDefaultServers(Network.REGTEST)).toBe(primary.regtestServers);
});

test("Should get electrum settings from DefaultProvider", async () => {
  config.DefaultProvider.servers.mainnet = ["wss://example.com:777"]

  expect(config.getDefaultServers(Network.MAINNET)).toStrictEqual(["wss://example.com:777"]);
  expect(config.getDefaultServers(Network.TESTNET)).toBe(primary.testnetServers);
  expect(config.getDefaultServers(Network.REGTEST)).toBe(primary.regtestServers);
...
test("Should get electrum settings from env", async () => {
  ...
  expect(config.getDefaultServers(Network.MAINNET)).toStrictEqual(["https://example.com:1234"]);
  expect(config.getDefaultServers(Network.TESTNET)).toStrictEqual([
    "https://test.example.com:1234",
    "https://test.example.dk:1234",
  ]);
  expect(config.getDefaultServers(Network.REGTEST)).toStrictEqual(["ws://reg.example.com:1234"]);
});
