...
export async function getAddressHistory(cashaddr:string, provider: NetworkProvider, unit="sat", start=0, count=25, collapseChange=true): Promise<TransactionHistoryI> {
    ...
    let txItemPromises = txnHashes.map( (tx) => { return getDetailedHistory(cashaddr, tx.tx_hash, tx.height, provider, collapseChange)})
...
export  async function getDetailedHistory(cashaddr:string, hash:string, height:number, provider:NetworkProvider, collapseChange:boolean): Promise<TransactionHistoryItemI[]>{
...
async function getMatchingInputs(transaction: Transaction, cashaddr:string, height:number, hash:string, provider, collapseChange){
    ...
    for(let input of transaction.inputs){
        ...
        if(binToHex(outpoint.lockingBytecode) === lockingBytecodeHex){
            ...
            for(let output of transaction.outputs){
                ...
                let from = lockingBytecodeToCashAddress(outpoint.lockingBytecode, prefix) as string
                ...
                if(binToHex(output.lockingBytecode)===lockingBytecodeHex){
                    ...
                    if(!txIds.find((str) => str === `${hash}:i:${idx}`)){
                        ...
                        let to = lockingBytecodeToCashAddress(output.lockingBytecode, prefix) as string
                        r.push(
                            {
                                "from": from,
                                "to": to,
    ...
    for(let output of transaction.outputs){
        ...
        if(binToHex(output.lockingBytecode)===lockingBytecodeHex){
            ...
            if(transaction.inputs.length==1){
                ...
                let from = transaction.inputs.map( i => `${binToHex(i.outpointTransactionHash)}:o:${i.outpointIndex}`).join(";")
                r.push(
                    {
                        "from": from,
                        "to": cashaddr,
...
async function getInputTransactions(transaction : Transaction, provider : NetworkProvider){
...
function getFee(inputUtxos:Output[], utxos:Output[], lockingBytecodeHex, collapseChange){
    let inValues = 0
    ...
    const outValues = utxos.map( utxo => Number(binToBigIntUint64LE(utxo.satoshis))).reduce((a: number, b: number) => a + b, 0)

    let fee = 0
    if(collapseChange){
        const nonChangeOutputs = utxos.map( output => binToHex(output.lockingBytecode)===lockingBytecodeHex ? 0: 1).reduce((a: number, b: number) => a + b, 0)
...
function applyBalance(preprocessedTxns:TransactionHistoryItemI[], currentBalance:number, unit:string, factor:number ): TransactionHistoryItemI[]{
