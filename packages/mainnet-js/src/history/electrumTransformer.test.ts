...
test("Should get an address history", async () => {
    ...
    if (!process.env.PRIVATE_WIF) {
      ...
      const bobHistory = (await getAddressHistory(bob.getDepositAddress(), bob.provider));
...
test("Should cut results with a longer history to given count", async () => {
  ...
  if (!process.env.PRIVATE_WIF) {
    ...
    // Build Bob's wallet from a public address, check his balance.
    const bobHistory = (await getAddressHistory(bob.getDepositAddress(), bob.provider, "sat", 0, 2));
...
test("Should handel input and fee from many utxos", async () => {
  ...
  if (!process.env.PRIVATE_WIF) {
    ...
    // Build Bob's wallet from a public address, check his balance.
    const bobHistory = (await getAddressHistory(bob.getDepositAddress(), bob.provider, "sat"));
