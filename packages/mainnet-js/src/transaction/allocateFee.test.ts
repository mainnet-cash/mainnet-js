...
test("Should get the regtest wallet balance", async () => {
  ...
  if (!process.env.ADDRESS) {
    ...
    const funder = await RegTestWallet.fromId(`wif:regtest:${process.env.PRIVATE_WIF!}`);
...
test("Expect first in to subtract fee from first", async () => {
    ...
    let allocatedInputs = await allocateFee(requests, fee, FeePaidByEnum.first, BigInt(0));
  ...
  test("Expect last to subtract fee from last", async () => {
    ...
    let fee = 1
    let requests = asSendRequestObject(to) as SendRequest[]
    let allocatedInputs = allocateFee(requests, fee, FeePaidByEnum.last, BigInt(0));
  ...
  test("Expect all to allocate fees equally", async () => {
    ...
    let allocatedInputs =  allocateFee(requests, fee, FeePaidByEnum.any, BigInt(0));
  ...
  test("Expect insufficient funds to error", async () => {
    ...
    expect.assertions(1);
      ...
      let fee = 7000
      let requests = asSendRequestObject(to) as SendRequest[]
        let allocatedInputs =  allocateFee(requests, fee, FeePaidByEnum.changeThenAny, BigInt(999));
      } catch (e: any) {
        expect(e.message).toBe(
          "Insufficient funds for transaction given fee"
        );
      }
  });
  ...
  test("Expect dust amounts to error", async () => {
    ...
    expect.assertions(1);
      ...
      let fee = 1500
      let requests = asSendRequestObject(to) as SendRequest[]
        let allocatedInputs =  allocateFee(requests, fee, FeePaidByEnum.first, BigInt(0));
      } catch (e: any) {
        expect(e.message).toBe(
          "Fee strategy would result in dust output"
        );
      }
  });
...
test("Expect `change,any` to consume only change", async () => {
  ...
  let fee = 1362
  let requests = asSendRequestObject(to) as SendRequest[]
  let result =  allocateFee(requests, fee, FeePaidByEnum.changeThenAny, BigInt(1362));
