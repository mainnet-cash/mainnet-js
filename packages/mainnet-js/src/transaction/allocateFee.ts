...
export function checkSatsAvailable(sendRequestArray: Array<SendRequest>, fee:number){
    let amountAvailable =  sendRequestArray.reduce(function (sum, r) {return sum + (r.value-DUST_UTXO_THRESHOLD)}, 0);
    if(amountAvailable < fee){
        throw Error("Insufficient funds for transaction given fee")
    }
}

export function checkForOpReturn(output:SendRequest| OpReturnData) : SendRequest{
...
export function sortSendRequests(sendRequestArray: Array<SendRequest>){
    return sendRequestArray.sort((a:SendRequest, b:SendRequest) => a.value - b.value);
}
...
function changeThenFallback(requests : Array <SendRequest| OpReturnData>, fee:number, change:bigint, fallbackFn: Function){
...
export function allocateFee(requests: Array<SendRequest| OpReturnData>, fee: number, feePaidBy: FeePaidByEnum, change: bigint,): Array<SendRequest>{
