...
describe(`Test Wallet library`, () => {
    ...
    test("Should get a testnet wallet fromId", async () => {
        let alice = await TestNetWallet.fromId(`wif:testnet:${process.env.PRIVATE_WIF}`);
    ...
    test("Should also throw error on wif/network mismatch", async () => {
        expect.assertions(1);
        try {
            await TestNetWallet.fromId(`wif:testnet:KysvoRyDkxQycBGj49K8oC3minAfoXnVmkcgx6UsZx3g2VvyGCAa`);
        }
        catch (e) {
            expect(e.message).toBe("Testnet type wif KysvoRyDkxQycBGj49K8oC3minAfoXnVmkcgx6UsZx3g2VvyGCAa passed, should start with c");
        }
    });
    ...
    test("Should get an error passing wrong network to fromId", async () => {
        ...
        try {
            ...
            expect(e.message.slice(0, 97)).toBe("Network prefix regtest to a testnet wallet");
        }
    });
    ...
    test("Should get the regtest wallet balance", async () => {
        ...
        if (!process.env.PRIVATE_WIF) {
            ...
            let alice = await RegTestWallet.fromId(`wif:regtest:${process.env.PRIVATE_WIF}`); // insert WIF from #1
            expect(await alice.getBalance("sat")).toBeGreaterThan(5000 * 100000000);
        }
    });
...
describe(`Tests named wallet creation`, () => {
    ...
    test("Expect force saving over a named wallet to fail", async () => {
        ...
        try {
            ...
            expect(e.message).toBe("A wallet with the name duplicate_name already exists in dup_test");
        }
    });
