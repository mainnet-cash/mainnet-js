...
describe(`Wallet subscriptions`, () => {
  ...
  describe(`Wallet extrema behavior regression testing`, () => {
    test(`Should operate very well above dust threshold (${DUST*3}), 'min relay fee not met (code 66)' regression`, async () => {
      const aliceWif = `wif:regtest:${process.env.PRIVATE_WIF!}`;
      const aliceWallet = await RegTestWallet.fromId(aliceWif);
      const bobWallet = await RegTestWallet.newRandom();
  ...
  test(`Should operate very well above dust threshold (${DUST*2}), 'min relay fee not met (code 66)' regression`, async () => {
    const aliceWif = `wif:regtest:${process.env.PRIVATE_WIF!}`;
    const aliceWallet = await RegTestWallet.fromId(aliceWif);
    const bobWallet = await RegTestWallet.newRandom();
    const charlieWallet = await RegTestWallet.newRandom();
  ...
  test(`Should operate well above dust threshold (${DUST+328}), 'min relay fee not met (code 66)' regression`, async () => {
    const aliceWif = `wif:regtest:${process.env.PRIVATE_WIF!}`;
    const aliceWallet = await RegTestWallet.fromId(aliceWif);
    const bobWallet = await RegTestWallet.newRandom();
    const charlieWallet = await RegTestWallet.newRandom();
  ...
  test(`Should operate slightly above dust threshold (${DUST+1}), 'min relay fee not met (code 66)' regression`, async () => {
    const aliceWif = `wif:regtest:${process.env.PRIVATE_WIF!}`;
    const aliceWallet = await RegTestWallet.fromId(aliceWif);
    const bobWallet = await RegTestWallet.newRandom();
    const charlieWallet = await RegTestWallet.newRandom();
  ...
  test(`Should operate with dust threshold (${DUST}), 'min relay fee not met (code 66)' regression`, async () => {
    ...
    test(`Should throw error with dust amounts (${DUST-1}), 'min relay fee not met (code 66)' regression`, async () => {
      expect.assertions(1);
      try {
        const aliceWif = `wif:regtest:${process.env.PRIVATE_WIF!}`;
        const aliceWallet = await RegTestWallet.fromId(aliceWif);
