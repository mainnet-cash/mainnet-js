...
describe(`SmartBchWallet should function in the browser`, () => {
  ...
  test("Test SmartBch sending", async () => {
    const result = await page.evaluate(async () => {
      ...
      expects.push([sendResult[0].balance.bch, "toBeGreaterThan", balance.bch - (0.1 + feeDelta)]);
      ...
      expects.push([sendManyResult[0].balance.bch, "toBeGreaterThan", (
        balance.bch - 3 * (0.1 + feeDelta)
      )]);
      expects.push([sendManyResult[1].balance.bch, "toBeGreaterThan", (
        balance.bch - 3 * (0.1 + feeDelta)
      )]);
