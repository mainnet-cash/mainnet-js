import { hexToBin,binToHex, instantiateRipemd160, instantiateSha256, instantiateSecp256k1 } from "@bitauth/libauth";
...
test("Should calculate public key hash using via hash160", async () => {
  ...
  let pkh2 = ripemd160.hash(sha256.hash(hexToBin("02b4632d08485ff1df2db55b9dafd23347d1c47a457072a1e87be26896549a8737")))
  expect(pkh2.length).toBe(20);
...
test("Should calculate public key hash consistent with hash160", async () => {
  ...
  let w = await Wallet.fromId("seed:mainnet:abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about")
...
test("Should return the a public key hash of manually set wallet", async () => {
  ...
  let w = new Wallet()
  w.privateKey = hexToBin("e284129cc0922579a535bbf4d1a3b25773090d28c909bc0fed73b5e0222cc372")
  // @ts-ignore
  await w.deriveInfo()
  let pkh = derivePublicKeyHash(w.getDepositAddress())
