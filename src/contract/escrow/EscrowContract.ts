import {
    Contract, SignatureTemplate, CashCompiler,
...
export class EscrowContract {
    ...
    public async run(wif: string, funcName: string, getHexOnly = false, utxos?: Utxo[]) {
        ...
        if (utxos.length > 0) {
            try {
                let funcInstance = func(fee, publicKey, sig).to(address, balance-fee).from(utxos);
