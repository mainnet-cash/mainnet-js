...
(async () => {
  ...
  let dockerArgs = [
    ...
    `-i`,
    `/local/swagger/v1/api.yml`,
    `-g`,
    command,
    `-o`,
    `/local/generated/client/${command}`,
  ];
  let additionalProperties = [];
  switch (command) {
    case "php":
      additionalProperties = [
        `--additional-properties=packageName=Mainnet,invokerPackage=Mainnet`,
      ];
      break;
    case "python":
      additionalProperties = [
        `--additional-properties=packageName=mainnet,projectName=mainnet`,
      ];
      break;
    default:
