...
async function checkBchdBinary() {
    ...
    if (fs.existsSync(`${BIN_DIRECTORY}/bchd`) && fs.existsSync(`${BIN_DIRECTORY}/bchctl`)) {
...
function getArchitecture() {
    ...
    if (architectures.has(process.arch)) {
        ...
        throw new Error("Your architecture is not recognized, please download bchd manually and move the unzipped contents to the  ./bin/ folder  ");
    }
}
...
function getPlatform() {
    ...
    if (platforms.has(process.platform)) {
        ...
        throw new Error("Your operating system is not recognized, please download bchd manually and move the unzipped contents to the  ./bin/ folder  ")
    }
}

async function downloadZipFile(url, downloadPath) {
    await unzipper.Open.url(request, url).then(d => d.extract({ path: downloadPath, concurrency: 5 }));
}
...
function symlinkBchdExecutibles(dir, zipDir, platform) {
    ...
    let executibles = ['bchd', 'bchctl'].map(b => platform === 'windows' ? b + '.exe' : b)
...
function isBchdVersionLatest() {
    ...
    const bchd = spawnSync(`${process.env.BCHD_BIN_DIRECTORY}/bchd`, [`--version`]);
